...
export async function genererPlanningPdf(options: {
  ...
  const buffer = await new Promise<Buffer>((resolve, reject) => {
    ...
    doc.moveDown(0.5);
    doc.font("Helvetica").fontSize(12).text(moisLabel, { align: "center" });
    doc.fontSize(10).fillColor("#444444").text(`Ligne(s) : ${lignesLabel}`, {
      align: "center",
    });
    ...
    for (const ligne of lignesFiltrees) {
      ...
      if (groupe.astreintes.length === 0) {
        ...
        doc.text("Aucune astreinte publiée sur ce mois.");
        doc.fillColor("#000000");
        doc.moveDown(0.5);
        continue;
      }
